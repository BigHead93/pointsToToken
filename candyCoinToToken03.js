//添加了动态计算gaslimit功能。value = web3.eth.estimateGas * 2
//可直接运行，运行参数 node 文件名 <wallet address> <token amount>
let fs =require("fs");
...
try{
	...
	if(toAddress && toAmount){
		...
		if(!web3.utils.isAddress(toAddress)){
			flag = false;
			console.log('The receiver\'s address is invalid');
		}
		if(isNaN(toAmount)){
			flag = false;
		...
		if(flag){
			...
			web3.eth.estimateGas({
					...
					console.log('estimate gas limit: ' + value * 2);		
					transfer(value * 2);
				});
		}
	}else{
