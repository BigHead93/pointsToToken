...
function transfer (_serialId, _address, _amount, _times) {
	if(_times == 5){
	...
	var toAmount = _amount * 1000000000000;
					...
					web3.eth.sendSignedTransaction('0x' + serializedTx.toString('hex'),
						function(err, hash) {
						    ...
						    if (!err) {
						    	...
						    	logger.error('Error in sendSignedTransaction: ' + err);
						    	if(hash == null){
							    	transfer(_serialId, _address, _amount, _times + 1);
						    	}else{
							    	logger.error('Error in sendSignedTransaction: TxHash: %s Error: %s - ', hash, err);
						    	}
